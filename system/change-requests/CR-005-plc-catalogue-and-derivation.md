...
qualifies for one. The plain-language summary is [D-05](../approved-deviations.md).

---

## 1. What the approved baseline says today

`prd-complete.md` is the binding baseline and it never enumerates PLC categories.

| Clause | Wording |
| --- | --- |
| `prd-complete.md` §16.3 | "PLC is percentage only." |
| `prd-complete.md` §16.3 | "Each distinct PLC component is charged once." |
| `prd-complete.md` §16.3 | "Same category on multiple sides is not charged repeatedly." |
| `prd-complete.md` §8.5 | "Use the visible term **Location Charge (PLC %)**." |
| `prd-complete.md` §16.2 | Plot fields include "North/South/East/West boundary", "Road width when Road", "Adjacent Plot Number when Plot boundary", "Park Facing", "Derived facing/open-side display" |
| `prd-complete.md` §23.1 | "Payment, PLC and commission percentages: exact decimal, up to four decimal places · Plot area: up to four decimal places" |

The developer specification goes further:

| Clause | Wording |
| --- | --- |
| `plc-location-charge.md` §3.2 | "The actual PLC categories and their percentages must come from authorised Project setup. Developers must not invent them." |
| `plc-location-charge.md` §3.3 | "The category code must remain stable across versions … `ROAD_FACING` / `PARK_FACING` / `CORNER`" |
| `plc-location-charge.md` §2.3 | "The calculation must deduplicate by a stable category key, not by the display label." |
| `plc-location-charge.md` §4.1 | Lists the Plot characteristics PLC may use, boundaries and road width among them |
| `plc-location-charge.md` §4.2 | Distinguishes a **derived** component from a **manual** one |
...
**Unaffected.** `prd-complete.md` §16.3's three rules all still hold. "Each distinct
...
**Unaffected.** `plc-location-charge.md` §2.3 requires deduplication by a stable key rather than
...
**Unaffected.** `plc-location-charge.md` §4.1 and §4.2 prefer derived applicability, and §4.1
already names boundaries and road width as inputs. This implements the preferred
route rather than departing from it.

**Contradicted.** `plc-location-charge.md` §3.2 — "Developers must not invent them". The four
categories are fixed in code. They were specified by the owner rather than
invented by development, and `prd-complete.md`, which outranks `plc-location-charge.md`, enumerates
no categories at all. The rule in `prd-complete.md`'s hierarchy §4 is that a
...
- `Plot.parkFacing` — a `PARK` boundary says the same thing once instead of
  twice. `prd-complete.md` §16.2 lists Park Facing as a Plot field and it is still
...
- `prd-complete.md` §16.2 calls the four boundaries, road width and adjacent Plot
  Number binding. No screen had ever collected them. The grid now does.
- `prd-complete.md` §8.4 lists **Edit Plot Details** as an action on an Available
  Plot. It had never been built. It now exists, under the §8.7 rules.
- `prd-complete.md` §8.1 — a Commercial Project cannot contain a Residential Plot
  Type. Nothing had enforced it. `prepareInventory` now does.
- `prd-complete.md` §23.1 — Plot area was stored to three decimal places, not four.
...
record applicability would be a second thing to keep in step. `plc-location-charge.md` §4.2's
